...
def Doolittle(A): #Dekomposisi matriks (membuat matriks Lower dan Upper) menggunakan A sebagai parameter
    ...
    for k in range(n): #iterasi dari 0 sampai n-1
...
def solve(A, b):
    ...
    for i in range(n): #iterasi untuk nilai i dari 0 sampai n-1
        ...
        #pengurangan antara elemen matriks b[i] dengan jumlah perkalian antara elemen L[i][j] dan y[j] 
        # untuk setiap j dari 0 sampai i-1
    for i in reversed(range(n)): #iterasi untuk nilai i dari n-1 sampai 0
        x[i] = (y[i] - sum(U[i][j] * x[j] for j in range(i + 1, n))) / U[i][i]
        #pengurangan antara elemen matriks y[i] dengan jumlah perkalian antara elemen U[i][j] dan x[j]
        # untuk setiap j dari i+1 sampai n-1. Kemudian, dibagi dengan nilai elemen diagonal utama U[i][i].
